...
if  [ $# -lt 2 ]; then
    echo "Parametros defaults, Sintaxe customizada: sh run-d.sh projectName debug|dev|tst|hml|prd"
    echo "Parametros defaults, default: sh run-d.sh projectName debug"
    projectName="poc"
    ambiente="debug"
else 
    projectName=$1
